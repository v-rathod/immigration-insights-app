...
describe.skipIf(!dataExists)("Pre-deploy: public/data/ artifacts", () => {
  ...
  // ── Optum shard CONTENT validation (catches consolidation pipeline failure) ──
  //
  // REGRESSION GUARD: Milestone 11.6 — "wage details not showing for any employer"
  // Root cause: consolidation step crashed on corrupted shards; wage_roles was
  // absent from all shards even though public/data shard file was large enough
  // to pass the size check above. Only reading the file catches this case.
  //
  // File size is NOT sufficient — an LCA-only shard (no wage data) can be > 500 KB.
  // Must read and validate actual field presence.

  it("Optum Services shard has wage_roles (consolidation ran, wage dashboard works)", () => {
    const path = join(PUBLIC_DATA, "employers", "78a46d3917846d886ef35fe989075cb353f21a1d.json");
    if (!existsSync(path)) return;
    const raw = readFileSync(path, "utf-8");
    const shard = JSON.parse(raw) as Record<string, unknown>;
    const wageRoles = shard.wage_roles as unknown[] | undefined;
    expect(Array.isArray(wageRoles)).toBe(true);
    expect((wageRoles ?? []).length).toBeGreaterThan(0);
  });

  it("Optum Services shard has wage_trend (salary chart works)", () => {
    const path = join(PUBLIC_DATA, "employers", "78a46d3917846d886ef35fe989075cb353f21a1d.json");
    if (!existsSync(path)) return;
    const shard = JSON.parse(readFileSync(path, "utf-8")) as Record<string, unknown>;
    expect(Array.isArray(shard.wage_trend)).toBe(true);
    expect((shard.wage_trend as unknown[]).length).toBeGreaterThan(0);
  });

  it("Optum Services shard has srs object (SRS dashboard works)", () => {
    const path = join(PUBLIC_DATA, "employers", "78a46d3917846d886ef35fe989075cb353f21a1d.json");
    if (!existsSync(path)) return;
    const shard = JSON.parse(readFileSync(path, "utf-8")) as Record<string, unknown>;
  ...
  // ── Monolithic wage JSON files (inputs to consolidation) ──────────────────
  //
  // These large files are INTERMEDIARY — generated by sync_wage_dashboard() and
  // consumed by consolidate_employer_shards() to embed per-employer salary data.
  // After consolidation they are DELETED (data embedded in shards, monoliths removed).
  //
  // We verify the shards have wage data embedded rather than checking for the
  // monolithic files, since the deployed state is post-consolidation.

  it("employer shards have embedded wage_roles data (post-consolidation)", () => {
    // Infosys (top employer) should have wage role data embedded in shard
    const path = join(PUBLIC_DATA, "employers", "d35dde19fc02ba7e3e8f71f25703e0ba6e70aa3d.json");
    if (!existsSync(path)) return;
    const shard = JSON.parse(readFileSync(path, "utf-8")) as Record<string, unknown>;
    expect(shard.wage_roles).toBeDefined();
    expect(Array.isArray(shard.wage_roles)).toBe(true);
    expect((shard.wage_roles as unknown[]).length).toBeGreaterThan(0);
  });

  it("employer shards have embedded wage_trend data (post-consolidation)", () => {
    const path = join(PUBLIC_DATA, "employers", "d35dde19fc02ba7e3e8f71f25703e0ba6e70aa3d.json");
    if (!existsSync(path)) return;
    const shard = JSON.parse(readFileSync(path, "utf-8")) as Record<string, unknown>;
    expect(shard.wage_trend).toBeDefined();
    expect(Array.isArray(shard.wage_trend)).toBe(true);
    expect((shard.wage_trend as unknown[]).length).toBeGreaterThan(0);
  });

  it("employer shards have embedded wage_role_trends data (post-consolidation)", () => {
    const path = join(PUBLIC_DATA, "employers", "d35dde19fc02ba7e3e8f71f25703e0ba6e70aa3d.json");
    if (!existsSync(path)) return;
    const shard = JSON.parse(readFileSync(path, "utf-8")) as Record<string, unknown>;
    expect(shard.wage_role_trends).toBeDefined();
    expect(Array.isArray(shard.wage_role_trends)).toBe(true);
    expect((shard.wage_role_trends as unknown[]).length).toBeGreaterThan(0);
  });
