...
describe.skipIf(!dataExists)("Pre-deploy: public/data/ artifacts", () => {
  ...
  // If these are missing, consolidation produces LCA-only shards (no wage data).
  //
  // Common failure mode: sync interrupted before wage step completes.
  // Fix: python3 scripts/sync_p2_data.py (or just the wage + consolidation steps)

  it("employer_role_profiles.json exists (wage_roles source for consolidation)", () => {
    const path = join(PUBLIC_DATA, "dashboards", "wage", "employer_role_profiles.json");
    expect(existsSync(path)).toBe(true);
  });

  it("employer_role_profiles.json is at least 50 MB (full dataset — not truncated)", () => {
    const path = join(PUBLIC_DATA, "dashboards", "wage", "employer_role_profiles.json");
    if (!existsSync(path)) return;
    // Full dataset is ~141 MB. If truncated during sync it might be a few KB.
    expect(fileSize(path)).toBeGreaterThan(50_000_000);
  });

  it("employer_salary_trend.json exists (wage_trend source for consolidation)", () => {
    const path = join(PUBLIC_DATA, "dashboards", "wage", "employer_salary_trend.json");
    expect(existsSync(path)).toBe(true);
  });

  it("employer_role_trends.json exists (wage_role_trends source for consolidation)", () => {
    const path = join(PUBLIC_DATA, "dashboards", "wage", "employer_role_trends.json");
    expect(existsSync(path)).toBe(true);
  });
